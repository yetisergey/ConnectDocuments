        ...
        public static void ExportToExcel(this DataTable dataTable, String filePath, bool overwiteFile = true)
        {
            ...
            using (OleDbConnection connection = new OleDbConnection(conn))
            {
                ...
                using (OleDbCommand command = new OleDbCommand())
                {
                    ...
                    command.CommandText = $"CREATE TABLE [{tableName.Replace("'","").Replace("$", "")}] ({String.Join(",", columnNames.Select(c => $"[{c}] VARCHAR").ToArray())});";
                    ...
                    foreach (DataRow row in dataTable.Rows)
                    {
                        ...
                        command.CommandText = $"INSERT INTO [{tableName.Replace("'", "").Replace("$", "")}]({String.Join(",", columnNames.Select(c => $"[{c}]"))}) VALUES ({String.Join(",", rowValues.Select(r => $"'{r}'").ToArray())});";
                        command.ExecuteNonQuery();
                    }
                }
